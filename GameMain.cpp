    ...
    case UpdateEngineState::WaitingForPress:
        if (m_controller->IsPressComplete())
        {
            ...
            case PressResultState::LoadGame:
                ...
                LoadAndFinalizeLevelInBackground();
                break;
...
winrt::fire_and_forget GameMain::LoadAndFinalizeLevelInBackground(bool renderNeeded)
{
    auto lifetime = get_strong();
...
winrt::fire_and_forget GameMain::HandleDeviceRestored()
{
    auto lifetime = get_strong();
    ...
    // into the correct state to reflect these cases.
    if (CoreWindow::GetForCurrentThread().Bounds().Width < GameUIConstants::MinPlayableWidth)
    {
        m_updateStateNext = m_updateState;
        m_updateState = UpdateEngineState::TooSmall;
        m_controller->Active(false);
...
void GameMain::ResetGame()
{
    ...
    LoadAndFinalizeLevelInBackground(/* renderNeeded */ true);
}
